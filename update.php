...
if($dateOut='' &&$dateIn>$dateOut){
    ...
    if($age>123 || strlen($name)<5){
           echo "<script>alert('Follow instructions properly');</script>";
    }else{
    $sql = "update crudoperation set name='$name', age=$age , disease='$disease', dateIn='$dateIn',dateOut='$dateOut' where id=$id";
   $result =  mysqli_query($con,$sql);
   if($result){
       echo "updated";
